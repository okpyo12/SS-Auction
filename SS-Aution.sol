...
contract Auction {
    ...
    //물건 상세 확인 함수
    function getProduct(uint256 _productId) public view returns (string memory, uint256, address, bool, address, uint256) {
        Product storage product = products[_productId];
        return (product.name, product.price, product.owner, product.sold, product.bidder, product.bid);
    }
}